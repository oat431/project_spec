...
tags: [coding-standards, style-guide, java, typescript, go, swebok]
standard_ref:
...
> Coding standards ensure consistent, readable, maintainable code across all services — regardless of language or framework. Standards are **enforced**, not suggested. If it doesn't pass linting in CI, it doesn't merge.

## 2. Language-Specific Standards

### 2.1 Java / Spring Boot (Flowero Discover, Gate, Guard)
...
```java
// ✅ Good — constructor injection, record DTO, meaningful names
...
// ❌ Bad — field injection, vague names, raw Exception
@Autowired
private EurekaServer e;   // field injection + meaningless name

try { ... } catch (Exception ex) { ... }  // too broad
```
...
### 2.2 TypeScript / Node.js (Tiny Mchwa, Fluffy Mouton)
...
All linting **must pass in CI** before merge. Configure as pre-commit hooks (Husky for Node.js, pre-commit for Go, Git hooks for Java).
...
|----------|-------------|
| [[031_README_developer_guide]] | Project setup and contribution |
| [[036_code_review_records]] | Standards enforced during review |
| [[062_coding_standards_security]] | Security-specific coding rules |

---

> **Template Standard:** Based on SWEBOK v4, Google Java Style Guide, Effective Go, Airbnb JS Style Guide
> **Usage:** Pick the language section matching your service. Standards are **enforced in CI** — linting failure = no merge. Standards without enforcement are just suggestions.
